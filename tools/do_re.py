...
class DoRegx():
    @staticmethod
    #定义一个方法，用正则表达式匹配有对应标识的字符串，匹配到之后将之替换，此处将之替换为GetData里面的同名变量的值
